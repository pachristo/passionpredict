...
//        }
    ...
    public function getPaymentStatus(Request $request, Subscription $subscription, \App\Solos\Modules\Transaction\Model\Transaction $logstrans)
    {
...
//            NOTE: CHECK IF USER IS CURRENTLY SUBSCRIBED. IF YES, ADD SUB TIME TO EXISTING NEXT_DUE_DATE. IF NO, CALCULATE NEXT_DUE_DATE FROM TODAY'S DATE AND UPGADE ACCCOUNT ACCORDINGLY
            ...
            User::where('id', currentUser()->id)->update(['subscription_id'=>$plan->id, 'subscription_type'=>$plan->planName, 'subscription_status'=>'1', 'date_subscribed'=>Carbon::now()->format('Y-m-d H:i:s'), 'next_due_date'=>$nextdue]);
            User::where('id', currentUser()->id)->increment('sub_count');

            $logstrans->create(['transactionRef'=>'Subscription Payment', 'transactionID'=>$result->id, 'transactionType'=>'Subscription', 'userID'=>currentUser()->id, 'planID'=>$plan->id, 'subDate'=>$today, 'statusCode'=>'approved', 'amountPaid'=>$plan->dollarPrice]);
