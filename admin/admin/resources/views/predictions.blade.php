...
{{--
    ...
    @if(trim($prediction->ice_hockey)!='') <span class="tag">Ice Hockey: <strong>{{$prediction->ice_hockey}} </strong></span> @endif --}}
            ...
            @if($prediction->threeFiveGoals!='') <span class="tag">3.5 Goals: <strong>{{$prediction->threeFiveGoals}} </strong></span> @endif
            @if($prediction->singleBet!='') <span class="tag">Single Bet : <strong>{{$prediction->singleBet}} </strong></span> @endif
            @if($prediction->weh!='') <span class="tag">Win Either Half : <strong>{{$prediction->weh}} </strong></span> @endif
            @if(trim($prediction->hwin)!='') <span class="tag">Home Win: <strong>{{$prediction->hwin}} </strong></span> @endif
