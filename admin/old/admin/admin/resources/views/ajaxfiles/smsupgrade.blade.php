<div class="container-fluid">
    ...
    <div class="col-sm-8">
        <h4>{{$user->full_name}} <br> <small>{{$user->email}} <br>
            ...
            @if($user->sms_due_date>date('Y-m-d H:i:s'))
                ...
                <b>EXPIRY DATE: </b>  {{\Carbon\Carbon::parse($user->sms_due_date)->format('d M, Y @ h:ia' )}} 
                {{-- ->format('l j M, Y - h:ia') --}}
                @else
                 <span class="text-danger">SUBSCRIPTION EXPIRED</span><br>
                <b>EXPIRY DATE: </b>  {{\Carbon\Carbon::parse($user->sms_due_date)->format('d M, Y @ h:ia' )}} 

            @endif

            @else
            <span class="text-danger">NEVER SUBSCRIBED</span><br>
    ...
    <div class="col-md-12">
        <hr>
        <form action="{{url('/smsacctupgrade')}}" method="POST" id="acctupgradeform">
